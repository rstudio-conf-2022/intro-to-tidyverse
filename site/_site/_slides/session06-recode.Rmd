...
# Parameters
file_patients <- here::here("slides/session06-recode/data/patients.csv")
...
1. Milestone recreation work time
...
Open this session's milestone file and complete the code in the chunk labelled `review-mutate`.
...
Let's begin with some review. Open the R Markdown (milestone?) file for today's session. Run the setup chunk, then use the `review-mutate` chunk to complete this review.
...
patients %>% 
  mutate(
    sex_recode = if_else(condition = ____, true = ____, false = ____)
  )
```
...
The first thing to add is the logical condition. This argument is called `condition`, but you'll rarely see that argument spelled out.
...
Finally we'll add the value the variable should take if the condition is `false`. Notice that the call to `mutate()` did not alter the `sex` column; it created a new column called `sex_recode`.
...
We want to identify which patients require medical intervention. Patients eligible for intervention must meet three criteria:
...
Add a variable named `intervene` which takes the value `“yes”` for patients who meet the criteria, and `"no"` for all others. Save the result as a new tibble called `patients_interventions`.
...
Fill in the blanks to create a new column called `sys_bp_status` using the function `case_when()`.
...
![](session06-recode/images/case_when.png)

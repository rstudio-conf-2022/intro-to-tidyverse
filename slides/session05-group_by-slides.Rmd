...
```{r two-groups-out, ref.label="two-groups", echo=FALSE}
```

???
We can also group by multiple columns by adding additional columns to `group_by()`. 
Notice we're grouped by two groups now

---

# Group by multiple columns

```{r two-group-summarize, eval=FALSE}
pollution %>% 
  group_by(continent, year) %>% #<<
...
```{r two-group-summarize-out, ref.label="two-group-summarize", echo=FALSE}
```

???
now, when we summarize, we're going to get one median for each continent year group
---
...
???
what if after we summarize, we find the find the row with the highest median?
going to be...
...
???
if we filter after summarize, we might think we'd just get back the row with the highest median value

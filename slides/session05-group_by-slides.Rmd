...
???
we've already seen a couple summary functions, including sum and min, but there are
...
<img src="images/session05/summarize-remove-group.png"/>
...
1. `group_by()` groups data

--

2. `group_by()` + `summarize()` summarizes data by group

--

3. `summarize()` removes one layer of grouping

--

4. `ungroup()` removes all groups
...
Open `session05-mini-project` in Academy. 
 
```{r echo = FALSE}
countdown::countdown(minutes = 40)
```

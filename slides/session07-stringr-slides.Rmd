...
To make this very clear, we have `r n_distinct(breed_traits$breed)` dog breeds (18 very good boys and girls are pictured here as an example)...
...
...and we want to subset the data so that we can continue our analysis on terrier breeds. Note that I don't know how many of the `r n_distinct(breed_traits$breed)` breeds in the dataset have "terrier" as part of their name.
...
And when I say "subset", alarm bells are probably going off in your head that we we'll be using the `filter()` function.

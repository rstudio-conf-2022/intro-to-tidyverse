...
Let's add a new column called `sex_recode` to our dataset. That means our `if_else()` statement should take place inside of `mutate()`.
...
**Recode the values of systolic blood pressure into two new categories: `"high"` and `"normal"`.** Save the result as a new variable named `sys_status`.
...
**Add a new variable to the `patients_interventions` dataset called `bmi_predict` that calculates a 10% reduction in BMI for patients who receive intervention.** 

For all other patients, have `bmi_predict` display the patient’s current BMI.
...
```{r echo = FALSE}
countdown::countdown(seconds = 30)
```
...
**Recode the values of `glucose` into 3 categories: `"non-diabetic"`, `"pre-diabetic"`, and `"diabetic"`.** 
...
**Add a new variable to `patients_2` called `followup` that displays the recommended number of months until follow up for each patient.** Including the appropriate missing value in your solution.
